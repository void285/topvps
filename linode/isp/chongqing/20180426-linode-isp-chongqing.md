#  重庆到Linode各机房的测速数据（20180426） 

## 导读

本文是[TopVPS主机网络测试报告系列](https://vps123.top/pingtest)的一部分，主要提供PING测试的响应速度、丢包率等数据，关于PING指令以及响应速度、丢包率的指标意义，请看[这里](https://vps123.top/what-is-ping.html)。

![重庆到Linode各机房的测速数据（20180426）](/images/thumbnails/Chongqing_to_linode.png)
...
本文提供20180426重庆到Linode各机房的PING测试结果，测速点的运营商线路为电信、联通、移动，有效样本点65个，无超时点。连通概况如下：大陆线路响应均值为280毫秒，最快的三个机房所在地为佛利蒙、东京、法兰克福，最慢的三个为纽瓦克、亚特兰大、伦敦；亚特兰大、新加坡、伦敦、达拉斯、法兰克福等6处丢包率较高。

[注册 Linode](https://vps123.top/go/linode/_btn1)

## 重庆图表数据

![大陆省份重庆到VPS提供商Linode各机房的ping测试数据统计图，包含响应值的柱状图以及丢包率的散点图，数据日期为20180426](/images/pingtests/linode_20180426/plot_isp_chongqing_linode_20180426.png)

### 全部运营商

重庆全部运营商到Linode各机房的测速数据 [20180426] 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率 | 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率  
---|---|---|---|---|---|---|---|---|---  
佛利蒙 | 2个 | 0 | 91ms | 1% | 新加坡 | 9个 | 0 | 289ms | 25%  
东京 | 8个 | 0 | 153ms | 3% | 纽瓦克 | 10个 | 0 | 324ms | 8%  
法兰克福 | 8个 | 0 | 241ms | 11% | 亚特兰大 | 10个 | 0 | 330ms | 27%  
均值 | 65个 | 0 | 280ms | 15% | 伦敦 | 8个 | 0 | 341ms | 20%  
达拉斯 | 10个 | 0 | 285ms | 16% |  |  |  |  |   
  
简评：本表展示了重庆的 **电信、联通、移动** 线路到Linode各机房的平均测速数据，这些数据绘制在前面柱状图的 **绿色** 柱位上；如果你常驻重庆，可从此表了解你常用的网络到Linode各个机房的平均响应速度以及丢包率。从表中数据可以看到， **佛利蒙、东京** 的响应速度小于250ms，且丢包率在5%以内，值得关注；如果你多数时间只使用某运营商的网络，后面的分运营商的表格数据对你更有参考价值。

### 电信

重庆电信到Linode各机房的测速数据 [20180426] 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率 | 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率  
---|---|---|---|---|---|---|---|---|---  
佛利蒙 | 2个 | 0 | 91ms | 1% | 新加坡 | 5个 | 0 | 276ms | 3%  
东京 | 4个 | 0 | 223ms | 6% | 纽瓦克 | 5个 | 0 | 340ms | 4%  
法兰克福 | 4个 | 0 | 193ms | 24% | 亚特兰大 | 6个 | 0 | 291ms | 16%  
均值 | 36个 | 0 | 258ms | 10% | 伦敦 | 4个 | 0 | 280ms | 21%  
达拉斯 | 6个 | 0 | 251ms | 1% |  |  |  |  |   
  
简评：如果你是来自重庆的 **电信** 用户，想运行Linode的VPS产品，用作科学上网、云存储、远程任务、游戏服务器等用途，建议重点关注部署在 **佛利蒙** 的机房；从数据看，你的网络到这里的连通速度、丢包率都比较不错。请慎重选择部署在 **法兰克福、东京** 的机房，即使这几个城市离你较近；因为响应均值虽然还不错，但丢包率较高。文章结尾处有本文视角的其他近期测评的链接，建议你多看几期以提升判断的准确度；如果看好了，就果断[去让机器跑起来](https://vps123.top/go/linode/_1)吧！

### 联通

重庆联通到Linode各机房的测速数据 [20180426] 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率 | 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率  
---|---|---|---|---|---|---|---|---|---  
佛利蒙 | - | - | - | - | 新加坡 | 1个 | 0 | 298ms | 41%  
东京 | 1个 | 0 | 119ms | 0 | 纽瓦克 | 1个 | 0 | 253ms | 0  
法兰克福 | 1个 | 0 | 281ms | 1% | 亚特兰大 | 1个 | 0 | 342ms | 36%  
均值 | 7个 | 0 | 288ms | 18% | 伦敦 | 1个 | 0 | 399ms | 17%  
达拉斯 | 1个 | 0 | 328ms | 35% |  |  |  |  |   
  
简评：如果你是来自重庆的 **联通** 用户，想运行Linode的VPS产品，用作科学上网、云存储、远程任务、游戏服务器等用途，建议重点关注部署在 **东京** 的机房；从数据看，你的网络到这里的连通速度、丢包率都比较不错。文章结尾处有本文视角的其他近期测评的链接，建议你多看几期以提升判断的准确度；如果看好了，就果断[去让机器跑起来](https://vps123.top/go/linode/_2)吧！

### 移动

重庆移动到Linode各机房的测速数据 [20180426] 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率 | 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率  
---|---|---|---|---|---|---|---|---|---  
佛利蒙 | - | - | - | - | 新加坡 | 3个 | 0 | 295ms | 31%  
东京 | 3个 | 0 | 116ms | 2% | 纽瓦克 | 4个 | 0 | 379ms | 21%  
法兰克福 | 3个 | 0 | 249ms | 10% | 亚特兰大 | 3个 | 0 | 359ms | 30%  
均值 | 22个 | 0 | 292ms | 19% | 伦敦 | 3个 | 0 | 344ms | 23%  
达拉斯 | 3个 | 0 | 276ms | 13% |  |  |  |  |   
  
简评：如果你是来自重庆的 **移动** 用户，想运行Linode的VPS产品，用作科学上网、云存储、远程任务、游戏服务器等用途，建议重点关注部署在 **东京** 的机房；从数据看，你的网络到这里的连通速度、丢包率都比较不错。请慎重选择部署在 **法兰克福** 的机房，即使此处离你较近；因为响应均值虽然还不错，但丢包率较高。文章结尾处有本文视角的其他近期测评的链接，建议你多看几期以提升判断的准确度；如果看好了，就果断[去让机器跑起来](https://vps123.top/go/linode/_3)吧！
...
本文最初发表于[重庆到Linode各机房的测速数据（20180426）](https://vps123.top/pingtest/20180426-linode-isp-chongqing.html)
