#  陕西到Linode各机房的测速数据（20180527） 

## 导读

本文是[TopVPS主机网络测试报告系列](https://vps123.top/pingtest)的一部分，主要提供PING测试的响应速度、丢包率等数据，关于PING指令以及响应速度、丢包率的指标意义，请看[这里](https://vps123.top/what-is-ping.html)。

![陕西到Linode各机房的测速数据（20180527）](/images/thumbnails/Shan3xi_to_linode.png)
...
测速点的运营商线路为电信、联通、移动，有效样本点260个，其中超时点2个。连通概况如下：大陆线路响应均值为250毫秒，最快的三个机房所在地为东京、佛利蒙、新加坡，最慢的三个为达拉斯、亚特兰大、伦敦；达拉斯、伦敦有响应超时情况；亚特兰大、达拉斯、东京、新加坡、伦敦丢包率较高。

[注册 Linode](https://vps123.top/go/linode/_btn1)

## 陕西图表数据

![大陆省份陕西到VPS提供商Linode各机房的ping测试数据统计图，包含响应值的柱状图以及丢包率的散点图，数据日期为20180527](/images/pingtests/linode_20180527/plot_isp_shan3xi_linode_20180527.png)

### 全部运营商

陕西全部运营商到Linode各机房的测速数据 [20180527] 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率 | 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率  
---|---|---|---|---|---|---|---|---|---  
东京 | 31个 | 0 | 172ms | 9% | 法兰克福 | 32个 | 0 | 272ms | 3%  
佛利蒙 | 33个 | 0 | 200ms | 4% | 达拉斯 | 32个 | 1个 | 279ms | 10%  
新加坡 | 33个 | 0 | 222ms | 9% | 亚特兰大 | 33个 | 0 | 289ms | 10%  
均值 | 260个 | 2个 | 250ms | 7% | 伦敦 | 32个 | 1个 | 301ms | 5%  
纽瓦克 | 34个 | 0 | 266ms | 3% |  |  |  |  |   
...
陕西电信到Linode各机房的测速数据 [20180527] 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率 | 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率  
---|---|---|---|---|---|---|---|---|---  
东京 | 12个 | 0 | 218ms | 21% | 法兰克福 | 12个 | 0 | 261ms | 3%  
佛利蒙 | 13个 | 0 | 195ms | 2% | 达拉斯 | 12个 | 0 | 247ms | 2%  
新加坡 | 13个 | 0 | 307ms | 16% | 亚特兰大 | 13个 | 0 | 264ms | 2%  
均值 | 102个 | 0 | 250ms | 6% | 伦敦 | 13个 | 0 | 247ms | 1%  
纽瓦克 | 14个 | 0 | 260ms | 3% |  |  |  |  |   
  
简评：如果你是来自陕西的 **电信** 用户，想运行Linode的VPS产品，用作科学上网、云存储、远程任务、游戏服务器等用途，建议重点关注部署在 **佛利蒙、达拉斯、伦敦** 的机房；从数据看，你的网络到这几处的连通速度、丢包率都比较不错。请慎重选择部署在 **东京** 的机房，即使此处离你较近；因为响应均值虽然还不错，但丢包率较高。文章结尾处有本文视角的其他近期测评的链接，建议你多看几期以提升判断的准确度；如果看好了，就果断[去让机器跑起来](https://vps123.top/go/linode/_1)吧！

### 联通

陕西联通到Linode各机房的测速数据 [20180527] 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率 | 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率  
---|---|---|---|---|---|---|---|---|---  
东京 | 10个 | 0 | 166ms | 5% | 法兰克福 | 11个 | 0 | 311ms | 2%  
佛利蒙 | 11个 | 0 | 191ms | 7% | 达拉斯 | 11个 | 1个 | 298ms | 18%  
新加坡 | 11个 | 0 | 216ms | 6% | 亚特兰大 | 11个 | 0 | 262ms | 9%  
均值 | 86个 | 2个 | 254ms | 8% | 伦敦 | 10个 | 1个 | 328ms | 10%  
纽瓦克 | 11个 | 0 | 264ms | 7% |  |  |  |  |   
  
简评：如果你是来自陕西的 **联通** 用户，想运行Linode的VPS产品，用作科学上网、云存储、远程任务、游戏服务器等用途，建议重点关注部署在 **东京** 的机房；从数据看，你的网络到这里的连通速度、丢包率都比较不错。请慎重选择部署在 **佛利蒙、新加坡** 的机房，即使这几个城市离你较近；因为响应均值虽然还不错，但丢包率较高。文章结尾处有本文视角的其他近期测评的链接，建议你多看几期以提升判断的准确度；如果看好了，就果断[去让机器跑起来](https://vps123.top/go/linode/_2)吧！

### 移动

陕西移动到Linode各机房的测速数据 [20180527] 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率 | 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率  
---|---|---|---|---|---|---|---|---|---  
东京 | 9个 | 0 | 133ms | 3% | 法兰克福 | 9个 | 0 | 245ms | 3%  
佛利蒙 | 9个 | 0 | 213ms | 3% | 达拉斯 | 9个 | 0 | 291ms | 11%  
新加坡 | 9个 | 0 | 143ms | 5% | 亚特兰大 | 9个 | 0 | 341ms | 21%  
均值 | 72个 | 0 | 246ms | 6% | 伦敦 | 9个 | 0 | 329ms | 5%  
纽瓦克 | 9个 | 0 | 273ms | 1% |  |  |  |  |   
...
本文最初发表于[陕西到Linode各机房的测速数据（20180527）](https://vps123.top/pingtest/20180527-linode-isp-shan3xi.html)
