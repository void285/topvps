...
上海全部运营商到Vultr各机房的测速数据 [20180514] 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率 | 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率  
---|---|---|---|---|---|---|---|---|---  
东京 | 7个 | 0 | 95ms | 6% | 巴黎 | 8个 | 0 | 262ms | 8%  
新加坡 | 9个 | 0 | 153ms | 4% | 新泽西 | 8个 | 0 | 270ms | 0  
洛杉矶 | 7个 | 0 | 179ms | 13% | 芝加哥 | 9个 | 0 | 270ms | 23%  
悉尼 | 9个 | 0 | 212ms | 6% | 伦敦 | 7个 | 0 | 272ms | 1%  
西雅图 | 9个 | 0 | 214ms | 16% | 达拉斯 | 7个 | 0 | 275ms | 2%  
硅谷 | 7个 | 0 | 231ms | 15% | 迈阿密 | 8个 | 0 | 282ms | 3%  
均值 | 119个 | 0 | 238ms | 15% | 法兰克福 | 7个 | 0 | 284ms | 1%  
阿姆斯特丹 | 9个 | 0 | 261ms | 23% | 亚特兰大 | 8个 | 0 | 308ms | 32%  
  
简评：本表展示了上海的 **电信、联通、移动** 线路到Vultr各机房的平均测速数据，这些数据绘制在前面柱状图的 **绿色** 柱位上；如果你常驻上海，可从此表了解你常用的网络到Vultr各个机房的平均响应速度以及丢包率。从表中数据可以看到， **新加坡** 的响应速度小于250ms，且丢包率在5%以内，值得关注；如果你多数时间只使用某运营商的网络，后面的分运营商的表格数据对你更有参考价值。

### 电信

上海电信到Vultr各机房的测速数据 [20180514] 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率 | 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率  
---|---|---|---|---|---|---|---|---|---  
东京 | 5个 | 0 | 97ms | 12% | 巴黎 | 5个 | 0 | 205ms | 0  
新加坡 | 6个 | 0 | 166ms | 9% | 新泽西 | 6个 | 0 | 258ms | 0  
洛杉矶 | 4个 | 0 | 167ms | 0 | 芝加哥 | 6个 | 0 | 250ms | 0  
悉尼 | 6个 | 0 | 208ms | 11% | 伦敦 | 5个 | 0 | 291ms | 3%  
西雅图 | 6个 | 0 | 180ms | 0 | 达拉斯 | 5个 | 0 | 265ms | 0  
硅谷 | 6个 | 0 | 220ms | 3% | 迈阿密 | 6个 | 0 | 267ms | 4%  
均值 | 83个 | 0 | 225ms | 3% | 法兰克福 | 5个 | 0 | 278ms | 2%  
阿姆斯特丹 | 6个 | 0 | 217ms | 0 | 亚特兰大 | 6个 | 0 | 285ms | 2%  
  
简评：如果你是来自上海的 **电信** 用户，想运行Vultr的VPS产品，用作科学上网、云存储、远程任务、游戏服务器等用途，建议重点关注部署在 **洛杉矶、西雅图、巴黎、阿姆斯特丹、硅谷、芝加哥** 的机房；从数据看，你的网络到这几处的连通速度、丢包率都比较不错。请慎重选择部署在 **东京、新加坡、悉尼** 的机房，即使这几个城市离你较近；因为响应均值虽然还不错，但丢包率较高。文章结尾处有本文视角的其他近期测评的链接，建议你多看几期以提升判断的准确度；如果看好了，就果断[去让机器跑起来](https://vps123.top/go/vultr/_1)吧！

### 联通

上海联通到Vultr各机房的测速数据 [20180514] 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率 | 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率  
---|---|---|---|---|---|---|---|---|---  
东京 | - | - | - | - | 巴黎 | 1个 | 0 | 360ms | 26%  
新加坡 | 1个 | 0 | 172ms | 2% | 新泽西 | - | - | - | -  
洛杉矶 | 1个 | 0 | 186ms | 27% | 芝加哥 | 1个 | 0 | 286ms | 67%  
悉尼 | 1个 | 0 | 223ms | 1% | 伦敦 | - | - | - | -  
西雅图 | 1个 | 0 | 202ms | 30% | 达拉斯 | - | - | - | -  
硅谷 | - | - | - | - | 迈阿密 | - | - | - | -  
均值 | 8个 | 0 | 253ms | 36% | 法兰克福 | - | - | - | -  
阿姆斯特丹 | 1个 | 0 | 327ms | 71% | 亚特兰大 | 1个 | 0 | 269ms | 69%  
  
简评：如果你是来自上海的 **联通** 用户，想运行Vultr的VPS产品，用作科学上网、云存储、远程任务、游戏服务器等用途，建议重点关注部署在 **新加坡、悉尼** 的机房；从数据看，你的网络到这几处的连通速度、丢包率都比较不错。请慎重选择部署在 **洛杉矶、西雅图** 的机房，即使这几个城市离你较近；因为响应均值虽然还不错，但丢包率较高。文章结尾处有本文视角的其他近期测评的链接，建议你多看几期以提升判断的准确度；如果看好了，就果断[去让机器跑起来](https://vps123.top/go/vultr/_2)吧！

### 移动

上海移动到Vultr各机房的测速数据 [20180514] 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率 | 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率  
---|---|---|---|---|---|---|---|---|---  
东京 | 2个 | 0 | 94ms | 0 | 巴黎 | 2个 | 0 | 222ms | 0  
新加坡 | 2个 | 0 | 123ms | 2% | 新泽西 | 2个 | 0 | 281ms | 1%  
洛杉矶 | 2个 | 0 | 185ms | 14% | 芝加哥 | 2个 | 0 | 276ms | 3%  
悉尼 | 2个 | 0 | 206ms | 5% | 伦敦 | 2个 | 0 | 252ms | 0  
西雅图 | 2个 | 0 | 259ms | 19% | 达拉斯 | 2个 | 0 | 285ms | 4%  
硅谷 | 1个 | 0 | 242ms | 27% | 迈阿密 | 2个 | 0 | 297ms | 2%  
均值 | 28个 | 0 | 237ms | 5% | 法兰克福 | 2个 | 0 | 290ms | 0  
阿姆斯特丹 | 2个 | 0 | 239ms | 0 | 亚特兰大 | 1个 | 0 | 370ms | 26%  
  
简评：如果你是来自上海的 **移动** 用户，想运行Vultr的VPS产品，用作科学上网、云存储、远程任务、游戏服务器等用途，建议重点关注部署在 **东京、新加坡、悉尼、巴黎、阿姆斯特丹** 的机房；从数据看，你的网络到这几处的连通速度、丢包率都比较不错。请慎重选择部署在 **洛杉矶、硅谷** 的机房，即使这几个城市离你较近；因为响应均值虽然还不错，但丢包率较高。文章结尾处有本文视角的其他近期测评的链接，建议你多看几期以提升判断的准确度；如果看好了，就果断[去让机器跑起来](https://vps123.top/go/vultr/_3)吧！
