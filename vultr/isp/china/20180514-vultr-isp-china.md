...
大陆全部运营商到Vultr各机房的测速数据 [20180514] 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率 | 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率  
---|---|---|---|---|---|---|---|---|---  
东京 | 941个 | 16个 | 139ms | 10% | 阿姆斯特丹 | 913个 | 11个 | 289ms | 4%  
新加坡 | 972个 | 13个 | 186ms | 7% | 达拉斯 | 938个 | 14个 | 289ms | 14%  
西雅图 | 962个 | 13个 | 220ms | 5% | 芝加哥 | 958个 | 16个 | 290ms | 9%  
洛杉矶 | 966个 | 19个 | 235ms | 12% | 法兰克福 | 868个 | 9个 | 293ms | 4%  
悉尼 | 948个 | 11个 | 244ms | 3% | 新泽西 | 978个 | 12个 | 314ms | 6%  
硅谷 | 972个 | 15个 | 258ms | 11% | 伦敦 | 947个 | 20个 | 321ms | 7%  
均值 | 14190个 | 214个 | 268ms | 8% | 迈阿密 | 937个 | 16个 | 325ms | 11%  
巴黎 | 945个 | 15个 | 288ms | 4% | 亚特兰大 | 945个 | 14个 | 330ms | 15%  
...
在这里， **西雅图、悉尼** 的响应速度小于250ms，且丢包率在5%以内，值得关注。如果你主要关心某运营商的网络概况，后面几个表格的数据更有参考价值。  
如果你打算将VPS用作科学上网、云存储、远程任务、游戏服务器等用途，请 **关注你所在的省份** 到Vultr各机房的测速数据，它们可以给你更有益的参考。

### 电信

大陆电信到Vultr各机房的测速数据 [20180514] 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率 | 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率  
---|---|---|---|---|---|---|---|---|---  
东京 | 398个 | 11个 | 135ms | 10% | 阿姆斯特丹 | 381个 | 5个 | 261ms | 1%  
新加坡 | 402个 | 8个 | 195ms | 7% | 达拉斯 | 387个 | 10个 | 273ms | 7%  
西雅图 | 399个 | 6个 | 227ms | 2% | 芝加哥 | 388个 | 5个 | 300ms | 3%  
洛杉矶 | 406个 | 15个 | 235ms | 9% | 法兰克福 | 368个 | 6个 | 263ms | 2%  
悉尼 | 400个 | 7个 | 252ms | 2% | 新泽西 | 412个 | 4个 | 332ms | 3%  
硅谷 | 404个 | 9个 | 271ms | 5% | 伦敦 | 394个 | 9个 | 328ms | 2%  
均值 | 5913个 | 122个 | 265ms | 5% | 迈阿密 | 390个 | 10个 | 336ms | 6%  
巴黎 | 392个 | 9个 | 260ms | 1% | 亚特兰大 | 392个 | 8个 | 317ms | 8%  
  
简评：如果你关注电信的网络到Vultr各机房的连通性，此表数据提供了一个不错的概览， **西雅图** 的整体的连通速度、丢包率都比较不错， **东京、新加坡、洛杉矶** 也值得关注，如果要选择一个机房用于科学上网、云存储、远程任务、游戏服务器等用途，请查看你所在的省份的更具体的测评。如果看好了，就果断[去让机器跑起来](https://vps123.top/go/vultr/_1)吧！

### 联通

大陆联通到Vultr各机房的测速数据 [20180514] 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率 | 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率  
---|---|---|---|---|---|---|---|---|---  
东京 | 280个 | 4个 | 152ms | 16% | 阿姆斯特丹 | 271个 | 4个 | 340ms | 7%  
新加坡 | 282个 | 4个 | 226ms | 12% | 达拉斯 | 275个 | 3个 | 257ms | 13%  
西雅图 | 288个 | 5个 | 205ms | 4% | 芝加哥 | 290个 | 8个 | 293ms | 14%  
洛杉矶 | 280个 | 3个 | 214ms | 7% | 法兰克福 | 260个 | 2个 | 349ms | 8%  
悉尼 | 278个 | 3个 | 240ms | 7% | 新泽西 | 277个 | 5个 | 295ms | 11%  
硅谷 | 284个 | 2个 | 230ms | 6% | 伦敦 | 278个 | 7个 | 352ms | 15%  
均值 | 4173个 | 64个 | 273ms | 10% | 迈阿密 | 281个 | 5个 | 308ms | 15%  
巴黎 | 275个 | 5个 | 330ms | 7% | 亚特兰大 | 274个 | 4个 | 287ms | 12%  
  
简评：如果你关注联通的网络到Vultr各机房的连通性，此表数据提供了一个不错的概览， **西雅图** 的整体的连通速度、丢包率都比较不错， **东京、洛杉矶、新加坡、硅谷、悉尼** 也值得关注，如果要选择一个机房用于科学上网、云存储、远程任务、游戏服务器等用途，请查看你所在的省份的更具体的测评。如果看好了，就果断[去让机器跑起来](https://vps123.top/go/vultr/_2)吧！

### 移动

大陆移动到Vultr各机房的测速数据 [20180514] 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率 | 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率  
---|---|---|---|---|---|---|---|---|---  
