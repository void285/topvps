...
大陆全部运营商到Vultr各机房的测速数据 [20180426] 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率 | 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率  
---|---|---|---|---|---|---|---|---|---  
东京 | 782个 | 4个 | 166ms | 15% | 达拉斯 | 768个 | 4个 | 271ms | 5%  
新加坡 | 837个 | 5个 | 200ms | 4% | 法兰克福 | 832个 | 4个 | 285ms | 2%  
洛杉矶 | 779个 | 5个 | 208ms | 1% | 亚特兰大 | 763个 | 7个 | 296ms | 4%  
硅谷 | 769个 | 6个 | 220ms | 2% | 阿姆斯特丹 | 827个 | 5个 | 299ms | 1%  
西雅图 | 759个 | 5个 | 236ms | 2% | 新泽西 | 798个 | 8个 | 303ms | 1%  
悉尼 | 774个 | 6个 | 240ms | 1% | 芝加哥 | 764个 | 8个 | 304ms | 3%  
均值 | 11855个 | 90个 | 262ms | 3% | 迈阿密 | 749个 | 7个 | 304ms | 3%  
巴黎 | 829个 | 6个 | 269ms | 1% | 伦敦 | 825个 | 10个 | 335ms | 4%  
...
在这里， **新加坡、洛杉矶、硅谷、西雅图、悉尼** 的响应速度小于250ms，且丢包率在5%以内，值得关注。如果你主要关心某运营商的网络概况，后面几个表格的数据更有参考价值。  
如果你打算将VPS用作科学上网、云存储、远程任务、游戏服务器等用途，请 **关注你所在的省份** 到Vultr各机房的测速数据，它们可以给你更有益的参考。

### 电信

大陆电信到Vultr各机房的测速数据 [20180426] 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率 | 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率  
---|---|---|---|---|---|---|---|---|---  
东京 | 322个 | 3个 | 206ms | 22% | 达拉斯 | 312个 | 3个 | 271ms | 3%  
新加坡 | 330个 | 4个 | 203ms | 8% | 法兰克福 | 329个 | 2个 | 276ms | 3%  
洛杉矶 | 318个 | 3个 | 224ms | 2% | 亚特兰大 | 312个 | 6个 | 297ms | 3%  
硅谷 | 315个 | 4个 | 239ms | 3% | 阿姆斯特丹 | 331个 | 4个 | 259ms | 0  
西雅图 | 310个 | 4个 | 259ms | 3% | 新泽西 | 322个 | 7个 | 335ms | 2%  
悉尼 | 316个 | 3个 | 273ms | 2% | 芝加哥 | 316个 | 5个 | 354ms | 4%  
均值 | 4791个 | 65个 | 274ms | 4% | 迈阿密 | 308个 | 6个 | 325ms | 3%  
巴黎 | 326个 | 5个 | 258ms | 0 | 伦敦 | 324个 | 6个 | 336ms | 2%  
  
简评：如果你关注电信的网络到Vultr各机房的连通性，此表数据提供了一个不错的概览， **洛杉矶、硅谷** 的整体的连通速度、丢包率都比较不错， **新加坡、东京** 也值得关注，如果要选择一个机房用于科学上网、云存储、远程任务、游戏服务器等用途，请查看你所在的省份的更具体的测评。如果看好了，就果断[去让机器跑起来](https://vps123.top/go/vultr/_1)吧！

### 联通

大陆联通到Vultr各机房的测速数据 [20180426] 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率 | 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率  
---|---|---|---|---|---|---|---|---|---  
东京 | 222个 | 1个 | 141ms | 15% | 达拉斯 | 225个 | 1个 | 232ms | 3%  
新加坡 | 237个 | 1个 | 248ms | 2% | 法兰克福 | 233个 | 2个 | 328ms | 3%  
洛杉矶 | 225个 | 1个 | 187ms | 1% | 亚特兰大 | 219个 | 1个 | 260ms | 3%  
硅谷 | 224个 | 2个 | 199ms | 1% | 阿姆斯特丹 | 236个 | 1个 | 323ms | 2%  
西雅图 | 223个 | 1个 | 213ms | 0 | 新泽西 | 227个 | 1个 | 278ms | 2%  
悉尼 | 231个 | 2个 | 219ms | 1% | 芝加哥 | 214个 | 2个 | 264ms | 3%  
均值 | 3396个 | 22个 | 254ms | 3% | 迈阿密 | 211个 | 1个 | 278ms | 3%  
巴黎 | 235个 | 1个 | 300ms | 1% | 伦敦 | 234个 | 4个 | 330ms | 8%  
  
简评：如果你关注联通的网络到Vultr各机房的连通性，此表数据提供了一个不错的概览， **洛杉矶、硅谷、西雅图、悉尼、达拉斯、新加坡** 的整体的连通速度、丢包率都比较不错， **东京** 也值得关注，如果要选择一个机房用于科学上网、云存储、远程任务、游戏服务器等用途，请查看你所在的省份的更具体的测评。如果看好了，就果断[去让机器跑起来](https://vps123.top/go/vultr/_2)吧！

### 移动

大陆移动到Vultr各机房的测速数据 [20180426] 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率 | 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率  
---|---|---|---|---|---|---|---|---|---  
