#  山东到Digital Ocean各机房的测速数据（20180918） 

## 导读

本文是[TopVPS主机网络测试报告系列](https://vps123.top/pingtest)的一部分，主要提供PING测试的响应速度、丢包率等数据，关于PING指令以及响应速度、丢包率的指标意义，请看[这里](https://vps123.top/what-is-ping.html)。

![山东到Digital Ocean各机房的测速数据（20180918）](/images/thumbnails/Shandong_to_digitalocean.png)
...
本文提供20180918山东到Digital Ocean各机房的PING测试结果，测速点的运营商线路为电信、联通、移动，有效样本点1080个，其中超时点1个。连通概况如下：大陆线路响应均值为287毫秒，最快的三个机房所在地为旧金山、新加坡、纽约，最慢的三个为法兰克福、阿姆斯特丹、班加罗尔；旧金山有响应超时情况；多伦多、法兰克福、旧金山、阿姆斯特丹、新加坡等7处丢包率较高。

[注册 Digital Ocean](https://vps123.top/go/do/_btn1)

## 山东图表数据

![大陆省份山东到VPS提供商Digital Ocean各机房的ping测试数据统计图，包含响应值的柱状图以及丢包率的散点图，数据日期为20180918](/images/pingtests/do_20180918/plot_isp_shandong_do_20180918.png)

### 全部运营商

山东全部运营商到Digital Ocean各机房的测速数据 [20180918] 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率 | 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率  
---|---|---|---|---|---|---|---|---|---  
旧金山 | 191个 | 1个 | 231ms | 12% | 伦敦 | 91个 | 0 | 308ms | 11%  
新加坡 | 93个 | 0 | 265ms | 11% | 法兰克福 | 94个 | 0 | 313ms | 13%  
纽约 | 254个 | 0 | 280ms | 9% | 阿姆斯特丹 | 183个 | 0 | 318ms | 11%  
多伦多 | 86个 | 0 | 280ms | 13% | 班加罗尔 | 88个 | 0 | 338ms | 0  
均值 | 1080个 | 1个 | 287ms | 10% |  |  |  |  |   
  
简评：本表展示了山东的 **电信、联通、移动** 线路到Digital Ocean各机房的平均测速数据，这些数据绘制在前面柱状图的 **绿色** 柱位上；如果你常驻山东，可从此表了解你常用的网络到Digital Ocean各个机房的平均响应速度以及丢包率。从表中数据可以看到，各机房所在城市的响应值、丢包率都较高（TopVPS推荐选用响应值低于250ms，丢包率在5%%以内的机房），请慎重选择；如果你多数时间只使用某运营商的网络，后面的分运营商的表格数据对你更有参考价值。

### 电信

山东电信到Digital Ocean各机房的测速数据 [20180918] 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率 | 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率  
---|---|---|---|---|---|---|---|---|---  
旧金山 | 57个 | 1个 | 233ms | 10% | 伦敦 | 28个 | 0 | 291ms | 9%  
新加坡 | 28个 | 0 | 307ms | 9% | 法兰克福 | 28个 | 0 | 314ms | 8%  
纽约 | 79个 | 0 | 282ms | 7% | 阿姆斯特丹 | 57个 | 0 | 312ms | 11%  
多伦多 | 29个 | 0 | 276ms | 6% | 班加罗尔 | 28个 | 0 | 368ms | 0  
均值 | 334个 | 1个 | 291ms | 8% |  |  |  |  |   
  
简评：如果你是来自山东的 **电信** 用户，想运行Digital Ocean的VPS产品，用作科学上网、云存储、远程任务、游戏服务器等用途，请慎重选择部署在 **旧金山** 的机房，即使此处离你较近；因为响应均值虽然还不错，但丢包率较高。文章结尾处有本文视角的其他近期测评的链接，建议你多看几期以提升判断的准确度；如果看好了，就果断[去让机器跑起来](https://vps123.top/go/do/_1)吧！

### 联通

山东联通到Digital Ocean各机房的测速数据 [20180918] 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率 | 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率  
---|---|---|---|---|---|---|---|---|---  
旧金山 | 69个 | 0 | 225ms | 18% | 伦敦 | 35个 | 0 | 359ms | 16%  
新加坡 | 34个 | 0 | 355ms | 17% | 法兰克福 | 34个 | 0 | 350ms | 19%  
纽约 | 88个 | 0 | 281ms | 13% | 阿姆斯特丹 | 67个 | 0 | 366ms | 19%  
多伦多 | 30个 | 0 | 285ms | 21% | 班加罗尔 | 30个 | 0 | 363ms | 0  
均值 | 387个 | 0 | 312ms | 16% |  |  |  |  |   
  
简评：如果你是来自山东的 **联通** 用户，想运行Digital Ocean的VPS产品，用作科学上网、云存储、远程任务、游戏服务器等用途，请慎重选择部署在 **旧金山** 的机房，即使此处离你较近；因为响应均值虽然还不错，但丢包率较高。文章结尾处有本文视角的其他近期测评的链接，建议你多看几期以提升判断的准确度；如果看好了，就果断[去让机器跑起来](https://vps123.top/go/do/_2)吧！

### 移动

山东移动到Digital Ocean各机房的测速数据 [20180918] 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率 | 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率  
---|---|---|---|---|---|---|---|---|---  
旧金山 | 65个 | 0 | 235ms | 6% | 伦敦 | 28个 | 0 | 273ms | 7%  
新加坡 | 31个 | 0 | 135ms | 6% | 法兰克福 | 32个 | 0 | 274ms | 11%  
纽约 | 87个 | 0 | 277ms | 7% | 阿姆斯特丹 | 59个 | 0 | 275ms | 4%  
多伦多 | 27个 | 0 | 278ms | 11% | 班加罗尔 | 30个 | 0 | 282ms | 0  
均值 | 359个 | 0 | 257ms | 6% |  |  |  |  |   
  
简评：如果你是来自山东的 **移动** 用户，想运行Digital Ocean的VPS产品，用作科学上网、云存储、远程任务、游戏服务器等用途，请慎重选择部署在 **新加坡、旧金山** 的机房，即使这几个城市离你较近；因为响应均值虽然还不错，但丢包率较高。文章结尾处有本文视角的其他近期测评的链接，建议你多看几期以提升判断的准确度；如果看好了，就果断[去让机器跑起来](https://vps123.top/go/do/_3)吧！
...
本文最初发表于[山东到Digital Ocean各机房的测速数据（20180918）](https://vps123.top/pingtest/20180918-digitalocean-isp-shandong.html)
