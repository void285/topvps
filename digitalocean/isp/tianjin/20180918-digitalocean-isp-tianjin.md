#  天津到Digital Ocean各机房的测速数据（20180918） 

## 导读

本文是[TopVPS主机网络测试报告系列](https://vps123.top/pingtest)的一部分，主要提供PING测试的响应速度、丢包率等数据，关于PING指令以及响应速度、丢包率的指标意义，请看[这里](https://vps123.top/what-is-ping.html)。

![天津到Digital Ocean各机房的测速数据（20180918）](/images/thumbnails/Tianjin_to_digitalocean.png)
...
本文提供20180918天津到Digital Ocean各机房的PING测试结果，测速点的运营商线路为电信、联通、移动，有效样本点70个，无超时点。连通概况如下：大陆线路响应均值为270毫秒，最快的三个机房所在地为新加坡、法兰克福、旧金山，最慢的三个为纽约、阿姆斯特丹、班加罗尔；伦敦丢包率较高。

[注册 Digital Ocean](https://vps123.top/go/do/_btn1)

## 天津图表数据

![大陆省份天津到VPS提供商Digital Ocean各机房的ping测试数据统计图，包含响应值的柱状图以及丢包率的散点图，数据日期为20180918](/images/pingtests/do_20180918/plot_isp_tianjin_do_20180918.png)

### 全部运营商

天津全部运营商到Digital Ocean各机房的测速数据 [20180918] 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率 | 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率  
---|---|---|---|---|---|---|---|---|---  
新加坡 | 6个 | 0 | 213ms | 2% | 均值 | 70个 | 0 | 270ms | 2%  
法兰克福 | 6个 | 0 | 244ms | 3% | 纽约 | 17个 | 0 | 289ms | 2%  
旧金山 | 12个 | 0 | 249ms | 4% | 阿姆斯特丹 | 11个 | 0 | 294ms | 4%  
多伦多 | 6个 | 0 | 257ms | 0 | 班加罗尔 | 6个 | 0 | 323ms | 0  
伦敦 | 6个 | 0 | 262ms | 5% |  |  |  |  |   
  
简评：本表展示了天津的 **电信、联通、移动** 线路到Digital Ocean各机房的平均测速数据，这些数据绘制在前面柱状图的 **绿色** 柱位上；如果你常驻天津，可从此表了解你常用的网络到Digital Ocean各个机房的平均响应速度以及丢包率。从表中数据可以看到， **新加坡、法兰克福、旧金山** 的响应速度小于250ms，且丢包率在5%以内，值得关注；如果你多数时间只使用某运营商的网络，后面的分运营商的表格数据对你更有参考价值。

### 电信

天津电信到Digital Ocean各机房的测速数据 [20180918] 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率 | 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率  
---|---|---|---|---|---|---|---|---|---  
新加坡 | 2个 | 0 | 275ms | 0 | 均值 | 24个 | 0 | 265ms | 2%  
法兰克福 | 2个 | 0 | 219ms | 0 | 纽约 | 6个 | 0 | 279ms | 2%  
旧金山 | 4个 | 0 | 246ms | 8% | 阿姆斯特丹 | 4个 | 0 | 272ms | 2%  
多伦多 | 2个 | 0 | 225ms | 0 | 班加罗尔 | 2个 | 0 | 358ms | 0  
伦敦 | 2个 | 0 | 233ms | 0 |  |  |  |  |   
  
简评：如果你是来自天津的 **电信** 用户，想运行Digital Ocean的VPS产品，用作科学上网、云存储、远程任务、游戏服务器等用途，建议重点关注部署在 **法兰克福、多伦多、伦敦** 的机房；从数据看，你的网络到这几处的连通速度、丢包率都比较不错。请慎重选择部署在 **旧金山** 的机房，即使此处离你较近；因为响应均值虽然还不错，但丢包率较高。文章结尾处有本文视角的其他近期测评的链接，建议你多看几期以提升判断的准确度；如果看好了，就果断[去让机器跑起来](https://vps123.top/go/do/_1)吧！

### 联通

天津联通到Digital Ocean各机房的测速数据 [20180918] 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率 | 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率  
---|---|---|---|---|---|---|---|---|---  
新加坡 | 2个 | 0 | 251ms | 0 | 均值 | 23个 | 0 | 288ms | 2%  
法兰克福 | 2个 | 0 | 254ms | 0 | 纽约 | 5个 | 0 | 317ms | 2%  
旧金山 | 4个 | 0 | 224ms | 3% | 阿姆斯特丹 | 4个 | 0 | 329ms | 1%  
多伦多 | 2个 | 0 | 265ms | 0 | 班加罗尔 | 2个 | 0 | 367ms | 0  
伦敦 | 2个 | 0 | 277ms | 10% |  |  |  |  |   
  
简评：如果你是来自天津的 **联通** 用户，想运行Digital Ocean的VPS产品，用作科学上网、云存储、远程任务、游戏服务器等用途，建议重点关注部署在 **旧金山** 的机房；从数据看，你的网络到这里的连通速度、丢包率都比较不错。文章结尾处有本文视角的其他近期测评的链接，建议你多看几期以提升判断的准确度；如果看好了，就果断[去让机器跑起来](https://vps123.top/go/do/_2)吧！

### 移动

天津移动到Digital Ocean各机房的测速数据 [20180918] 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率 | 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率  
---|---|---|---|---|---|---|---|---|---  
新加坡 | 2个 | 0 | 115ms | 8% | 均值 | 23个 | 0 | 257ms | 3%  
法兰克福 | 2个 | 0 | 260ms | 9% | 纽约 | 6个 | 0 | 270ms | 1%  
旧金山 | 4个 | 0 | 278ms | 0 | 阿姆斯特丹 | 3个 | 0 | 281ms | 11%  
多伦多 | 2个 | 0 | 281ms | 0 | 班加罗尔 | 2个 | 0 | 245ms | 0  
伦敦 | 2个 | 0 | 275ms | 4% |  |  |  |  |   
  
简评：如果你是来自天津的 **移动** 用户，想运行Digital Ocean的VPS产品，用作科学上网、云存储、远程任务、游戏服务器等用途，建议重点关注部署在 **班加罗尔** 的机房；从数据看，你的网络到这里的连通速度、丢包率都比较不错。请慎重选择部署在 **新加坡** 的机房，即使此处离你较近；因为响应均值虽然还不错，但丢包率较高。文章结尾处有本文视角的其他近期测评的链接，建议你多看几期以提升判断的准确度；如果看好了，就果断[去让机器跑起来](https://vps123.top/go/do/_3)吧！
...
本文最初发表于[天津到Digital Ocean各机房的测速数据（20180918）](https://vps123.top/pingtest/20180918-digitalocean-isp-tianjin.html)
