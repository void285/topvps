...
![广西到Digital Ocean各机房的测速数据（20180622）](/images/thumbnails/Guangxi_to_digitalocean.png)
...
![大陆省份广西到VPS提供商Digital Ocean各机房的ping测试数据统计图，包含响应值的柱状图以及丢包率的散点图，数据日期为20180622](/images/pingtests/do_20180622/plot_isp_guangxi_do_20180622.png)

### 全部运营商

广西全部运营商到Digital Ocean各机房的测速数据 [20180622] 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率 | 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率  
---|---|---|---|---|---|---|---|---|---  
新加坡 | 14个 | 1个 | 221ms | 9% | 法兰克福 | 44个 | 0 | 278ms | 1%  
旧金山 | 86个 | 1个 | 225ms | 2% | 纽约 | 96个 | 0 | 292ms | 1%  
多伦多 | 42个 | 0 | 249ms | 1% | 阿姆斯特丹 | 87个 | 1个 | 300ms | 2%  
伦敦 | 40个 | 0 | 262ms | 1% | 班加罗尔 | 44个 | 0 | 343ms | 0  
均值 | 453个 | 3个 | 276ms | 2% |  |  |  |  |   
...
广西电信到Digital Ocean各机房的测速数据 [20180622] 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率 | 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率  
---|---|---|---|---|---|---|---|---|---  
新加坡 | 9个 | 1个 | 348ms | 29% | 法兰克福 | 19个 | 0 | 243ms | 1%  
旧金山 | 36个 | 0 | 263ms | 4% | 纽约 | 45个 | 0 | 304ms | 2%  
多伦多 | 19个 | 0 | 241ms | 2% | 阿姆斯特丹 | 37个 | 0 | 288ms | 1%  
伦敦 | 17个 | 0 | 230ms | 2% | 班加罗尔 | 19个 | 0 | 400ms | 1%  
均值 | 201个 | 1个 | 285ms | 3% |  |  |  |  |   
  
简评：如果你是来自广西的 **电信** 用户，想运行Digital Ocean的VPS产品，用作科学上网、云存储、远程任务、游戏服务器等用途，建议重点关注部署在 **伦敦、多伦多、法兰克福** 的机房；从数据看，你的网络到这几处的连通速度、丢包率都比较不错。文章结尾处有本文视角的其他近期测评的链接，建议你多看几期以提升判断的准确度；如果看好了，就果断[去让机器跑起来](https://vps123.top/go/do/_1)吧！

### 联通

广西联通到Digital Ocean各机房的测速数据 [20180622] 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率 | 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率  
---|---|---|---|---|---|---|---|---|---  
新加坡 | 1个 | 0 | 260ms | 0 | 法兰克福 | 12个 | 0 | 300ms | 1%  
旧金山 | 24个 | 1个 | 222ms | 1% | 纽约 | 22个 | 0 | 315ms | 2%  
多伦多 | 10个 | 0 | 263ms | 1% | 阿姆斯特丹 | 24个 | 1个 | 316ms | 1%  
伦敦 | 10个 | 0 | 290ms | 1% | 班加罗尔 | 12个 | 0 | 377ms | 0  
均值 | 115个 | 2个 | 294ms | 1% |  |  |  |  |   
  
简评：如果你是来自广西的 **联通** 用户，想运行Digital Ocean的VPS产品，用作科学上网、云存储、远程任务、游戏服务器等用途，建议重点关注部署在 **旧金山** 的机房；从数据看，你的网络到这里的连通速度、丢包率都比较不错。文章结尾处有本文视角的其他近期测评的链接，建议你多看几期以提升判断的准确度；如果看好了，就果断[去让机器跑起来](https://vps123.top/go/do/_2)吧！

### 移动

广西移动到Digital Ocean各机房的测速数据 [20180622] 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率 | 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率  
---|---|---|---|---|---|---|---|---|---  
新加坡 | 4个 | 0 | 54ms | 0 | 法兰克福 | 13个 | 0 | 291ms | 2%  
旧金山 | 26个 | 0 | 191ms | 1% | 纽约 | 29个 | 0 | 257ms | 0  
多伦多 | 13个 | 0 | 243ms | 0 | 阿姆斯特丹 | 26个 | 0 | 297ms | 4%  
伦敦 | 13个 | 0 | 265ms | 0 | 班加罗尔 | 13个 | 0 | 251ms | 0  
均值 | 137个 | 0 | 248ms | 1% |  |  |  |  |   
...
本文最初发表于[广西到Digital Ocean各机房的测速数据（20180622）](https://vps123.top/pingtest/20180622-digitalocean-isp-guangxi.html)
