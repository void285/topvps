#  湖南到Digital Ocean各机房的测速数据（20180918） 

## 导读

本文是[TopVPS主机网络测试报告系列](https://vps123.top/pingtest)的一部分，主要提供PING测试的响应速度、丢包率等数据，关于PING指令以及响应速度、丢包率的指标意义，请看[这里](https://vps123.top/what-is-ping.html)。

![湖南到Digital Ocean各机房的测速数据（20180918）](/images/thumbnails/Hunan_to_digitalocean.png)
...
本文提供20180918湖南到Digital Ocean各机房的PING测试结果，测速点的运营商线路为电信、联通、移动，有效样本点796个，其中超时点2个。连通概况如下：大陆线路响应均值为274毫秒，最快的三个机房所在地为新加坡、旧金山、多伦多，最慢的三个为纽约、阿姆斯特丹、班加罗尔；旧金山、纽约有响应超时情况；法兰克福、阿姆斯特丹、伦敦、旧金山丢包率较高。

[注册 Digital Ocean](https://vps123.top/go/do/_btn1)

## 湖南图表数据

![大陆省份湖南到VPS提供商Digital Ocean各机房的ping测试数据统计图，包含响应值的柱状图以及丢包率的散点图，数据日期为20180918](/images/pingtests/do_20180918/plot_isp_hunan_do_20180918.png)

### 全部运营商

湖南全部运营商到Digital Ocean各机房的测速数据 [20180918] 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率 | 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率  
---|---|---|---|---|---|---|---|---|---  
新加坡 | 67个 | 0 | 212ms | 3% | 伦敦 | 63个 | 0 | 277ms | 8%  
旧金山 | 139个 | 1个 | 246ms | 7% | 纽约 | 196个 | 1个 | 290ms | 3%  
多伦多 | 61个 | 0 | 269ms | 4% | 阿姆斯特丹 | 135个 | 0 | 291ms | 9%  
均值 | 796个 | 2个 | 274ms | 6% | 班加罗尔 | 67个 | 0 | 329ms | 0  
法兰克福 | 68个 | 0 | 276ms | 10% |  |  |  |  |   
  
简评：本表展示了湖南的 **电信、联通、移动** 线路到Digital Ocean各机房的平均测速数据，这些数据绘制在前面柱状图的 **绿色** 柱位上；如果你常驻湖南，可从此表了解你常用的网络到Digital Ocean各个机房的平均响应速度以及丢包率。从表中数据可以看到， **新加坡** 的响应速度小于250ms，且丢包率在5%以内，值得关注；如果你多数时间只使用某运营商的网络，后面的分运营商的表格数据对你更有参考价值。

### 电信

湖南电信到Digital Ocean各机房的测速数据 [20180918] 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率 | 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率  
---|---|---|---|---|---|---|---|---|---  
新加坡 | 24个 | 0 | 260ms | 1% | 伦敦 | 21个 | 0 | 262ms | 10%  
旧金山 | 50个 | 1个 | 237ms | 8% | 纽约 | 64个 | 0 | 298ms | 4%  
多伦多 | 20个 | 0 | 263ms | 4% | 阿姆斯特丹 | 48个 | 0 | 274ms | 4%  
均值 | 274个 | 1个 | 271ms | 5% | 班加罗尔 | 24个 | 0 | 351ms | 0  
法兰克福 | 23个 | 0 | 259ms | 7% |  |  |  |  |   
  
简评：如果你是来自湖南的 **电信** 用户，想运行Digital Ocean的VPS产品，用作科学上网、云存储、远程任务、游戏服务器等用途，请慎重选择部署在 **旧金山** 的机房，即使此处离你较近；因为响应均值虽然还不错，但丢包率较高。文章结尾处有本文视角的其他近期测评的链接，建议你多看几期以提升判断的准确度；如果看好了，就果断[去让机器跑起来](https://vps123.top/go/do/_1)吧！

### 联通

湖南联通到Digital Ocean各机房的测速数据 [20180918] 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率 | 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率  
---|---|---|---|---|---|---|---|---|---  
新加坡 | 24个 | 0 | 225ms | 8% | 伦敦 | 24个 | 0 | 310ms | 8%  
旧金山 | 48个 | 0 | 248ms | 12% | 纽约 | 73个 | 1个 | 293ms | 5%  
多伦多 | 24个 | 0 | 269ms | 7% | 阿姆斯特丹 | 48个 | 0 | 324ms | 14%  
均值 | 289个 | 1个 | 288ms | 9% | 班加罗尔 | 24个 | 0 | 337ms | 0  
法兰克福 | 24个 | 0 | 309ms | 13% |  |  |  |  |   
  
简评：如果你是来自湖南的 **联通** 用户，想运行Digital Ocean的VPS产品，用作科学上网、云存储、远程任务、游戏服务器等用途，请慎重选择部署在 **新加坡、旧金山** 的机房，即使这几个城市离你较近；因为响应均值虽然还不错，但丢包率较高。文章结尾处有本文视角的其他近期测评的链接，建议你多看几期以提升判断的准确度；如果看好了，就果断[去让机器跑起来](https://vps123.top/go/do/_2)吧！

### 移动

湖南移动到Digital Ocean各机房的测速数据 [20180918] 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率 | 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率  
---|---|---|---|---|---|---|---|---|---  
新加坡 | 19个 | 0 | 152ms | 1% | 伦敦 | 18个 | 0 | 259ms | 7%  
旧金山 | 41个 | 0 | 252ms | 3% | 纽约 | 59个 | 0 | 279ms | 1%  
多伦多 | 17个 | 0 | 275ms | 2% | 阿姆斯特丹 | 39个 | 0 | 273ms | 9%  
均值 | 233个 | 0 | 261ms | 4% | 班加罗尔 | 19个 | 0 | 300ms | 0  
法兰克福 | 21个 | 0 | 259ms | 9% |  |  |  |  |   
  
简评：如果你是来自湖南的 **移动** 用户，想运行Digital Ocean的VPS产品，用作科学上网、云存储、远程任务、游戏服务器等用途，建议重点关注部署在 **新加坡** 的机房；从数据看，你的网络到这里的连通速度、丢包率都比较不错。文章结尾处有本文视角的其他近期测评的链接，建议你多看几期以提升判断的准确度；如果看好了，就果断[去让机器跑起来](https://vps123.top/go/do/_3)吧！
...
本文最初发表于[湖南到Digital Ocean各机房的测速数据（20180918）](https://vps123.top/pingtest/20180918-digitalocean-isp-hunan.html)
