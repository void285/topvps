#  辽宁到Digital Ocean各机房的测速数据（20180918） 
...
本文提供20180918辽宁到Digital Ocean各机房的PING测试结果，测速点的运营商线路为电信、联通、移动，有效样本点666个，其中超时点4个。连通概况如下：大陆线路响应均值为283毫秒，最快的三个机房所在地为法兰克福、旧金山、伦敦，最慢的三个为多伦多、新加坡、班加罗尔；法兰克福、旧金山、阿姆斯特丹、多伦多有响应超时情况；新加坡、旧金山、法兰克福、阿姆斯特丹、多伦多等7处丢包率较高。

[注册 Digital Ocean](https://vps123.top/go/do/_btn1)

## 辽宁图表数据

![大陆省份辽宁到VPS提供商Digital Ocean各机房的ping测试数据统计图，包含响应值的柱状图以及丢包率的散点图，数据日期为20180918](/images/pingtests/do_20180918/plot_isp_liaoning_do_20180918.png)

### 全部运营商

辽宁全部运营商到Digital Ocean各机房的测速数据 [20180918] 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率 | 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率  
---|---|---|---|---|---|---|---|---|---  
法兰克福 | 60个 | 1个 | 267ms | 12% | 阿姆斯特丹 | 112个 | 1个 | 291ms | 11%  
旧金山 | 123个 | 1个 | 267ms | 14% | 多伦多 | 49个 | 1个 | 292ms | 10%  
伦敦 | 46个 | 0 | 270ms | 6% | 新加坡 | 55个 | 0 | 313ms | 20%  
均值 | 666个 | 4个 | 283ms | 10% | 班加罗尔 | 57个 | 0 | 339ms | 0  
纽约 | 164个 | 0 | 285ms | 8% |  |  |  |  |   
...
辽宁电信到Digital Ocean各机房的测速数据 [20180918] 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率 | 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率  
---|---|---|---|---|---|---|---|---|---  
法兰克福 | 25个 | 1个 | 234ms | 4% | 阿姆斯特丹 | 47个 | 1个 | 288ms | 4%  
旧金山 | 52个 | 0 | 284ms | 14% | 多伦多 | 19个 | 0 | 289ms | 2%  
伦敦 | 19个 | 0 | 268ms | 2% | 新加坡 | 23个 | 0 | 334ms | 7%  
均值 | 277个 | 2个 | 286ms | 6% | 班加罗尔 | 23个 | 0 | 405ms | 1%  
纽约 | 69个 | 0 | 297ms | 5% |  |  |  |  |   
...
辽宁联通到Digital Ocean各机房的测速数据 [20180918] 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率 | 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率  
---|---|---|---|---|---|---|---|---|---  
法兰克福 | 26个 | 0 | 300ms | 18% | 阿姆斯特丹 | 49个 | 0 | 308ms | 20%  
旧金山 | 53个 | 1个 | 253ms | 16% | 多伦多 | 25个 | 1个 | 290ms | 22%  
伦敦 | 21个 | 0 | 289ms | 15% | 新加坡 | 24个 | 0 | 368ms | 27%  
均值 | 296个 | 2个 | 291ms | 16% | 班加罗尔 | 26个 | 0 | 301ms | 0  
纽约 | 72个 | 0 | 280ms | 13% |  |  |  |  |   
  
简评：如果你是来自辽宁的 **联通** 用户，想运行Digital Ocean的VPS产品，用作科学上网、云存储、远程任务、游戏服务器等用途，很遗憾从本次测试来看，没有值得你关注的机房；请查看往期的测试，如果数据一直不乐观，你应考虑换用其他VPS提供商如[Vultr](https://vps123.top/go/vultr/_2)、[BandwagonHost](https://vps123.top/go/bandwagon/_3)、[Linode](https://vps123.top/go/linode/_4)等。

### 移动

辽宁移动到Digital Ocean各机房的测速数据 [20180918] 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率 | 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率  
---|---|---|---|---|---|---|---|---|---  
法兰克福 | 9个 | 0 | 267ms | 15% | 阿姆斯特丹 | 16个 | 0 | 276ms | 10%  
旧金山 | 18个 | 0 | 264ms | 11% | 多伦多 | 5个 | 0 | 297ms | 5%  
伦敦 | 6个 | 0 | 253ms | 2% | 新加坡 | 8个 | 0 | 236ms | 26%  
均值 | 93个 | 0 | 272ms | 9% | 班加罗尔 | 8个 | 0 | 312ms | 0  
纽约 | 23个 | 0 | 277ms | 5% |  |  |  |  |   
  
简评：如果你是来自辽宁的 **移动** 用户，想运行Digital Ocean的VPS产品，用作科学上网、云存储、远程任务、游戏服务器等用途，请慎重选择部署在 **新加坡** 的机房，即使此处离你较近；因为响应均值虽然还不错，但丢包率较高。文章结尾处有本文视角的其他近期测评的链接，建议你多看几期以提升判断的准确度；如果看好了，就果断[去让机器跑起来](https://vps123.top/go/do/_5)吧！
...
## 相关测速报告
  ...
  * 本期报告：辽宁到 _其他VPS提供商_ 各机房的测速报告： 
    * [Vultr](/vultr/isp/liaoning/20180918-vultr-isp-liaoning.md "辽宁到Vultr各机房的Ping测试 20180918")
...
本文最初发表于[辽宁到Digital Ocean各机房的测速数据（20180918）](https://vps123.top/pingtest/20180918-digitalocean-isp-liaoning.html)
