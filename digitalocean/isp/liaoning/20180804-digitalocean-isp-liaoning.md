#  辽宁到Digital Ocean各机房的测速数据（20180804） 

## 导读

本文是[TopVPS主机网络测试报告系列](https://vps123.top/pingtest)的一部分，主要提供PING测试的响应速度、丢包率等数据，关于PING指令以及响应速度、丢包率的指标意义，请看[这里](https://vps123.top/what-is-ping.html)。

![辽宁到Digital Ocean各机房的测速数据（20180804）](/images/thumbnails/Liaoning_to_digitalocean.png)
...
测速点的运营商线路为电信、联通、移动，有效样本点2509个，其中超时点33个。连通概况如下：大陆线路响应均值为299毫秒，最快的三个机房所在地为法兰克福、旧金山、伦敦，最慢的三个为纽约、班加罗尔、新加坡；旧金山、阿姆斯特丹、新加坡、法兰克福、纽约等6处有响应超时情况；新加坡、阿姆斯特丹、旧金山、纽约丢包率较高。

[注册 Digital Ocean](https://vps123.top/go/do/_btn1)

## 辽宁图表数据

![大陆省份辽宁到VPS提供商Digital Ocean各机房的ping测试数据统计图，包含响应值的柱状图以及丢包率的散点图，数据日期为20180804](/images/pingtests/do_20180804/plot_isp_liaoning_do_20180804.png)

### 全部运营商

辽宁全部运营商到Digital Ocean各机房的测速数据 [20180804] 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率 | 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率  
---|---|---|---|---|---|---|---|---|---  
法兰克福 | 206个 | 3个 | 264ms | 2% | 均值 | 2509个 | 33个 | 299ms | 5%  
旧金山 | 405个 | 9个 | 277ms | 7% | 纽约 | 622个 | 3个 | 310ms | 5%  
伦敦 | 210个 | 0 | 280ms | 1% | 班加罗尔 | 210个 | 3个 | 333ms | 2%  
多伦多 | 218个 | 0 | 288ms | 3% | 新加坡 | 210个 | 6个 | 350ms | 11%  
阿姆斯特丹 | 428个 | 9个 | 292ms | 9% |  |  |  |  |   
  
简评：本表展示了辽宁的 **电信、联通、移动** 线路到Digital Ocean各机房的平均测速数据，这些数据绘制在前面柱状图的 **绿色** 柱位上；如果你常驻辽宁，可从此表了解你常用的网络到Digital Ocean各个机房的平均响应速度以及丢包率。从表中数据可以看到，各机房所在城市的响应值、丢包率都较高（TopVPS推荐选用响应值低于250ms，丢包率在5%%以内的机房），请慎重选择；如果你多数时间只使用某运营商的网络，后面的分运营商的表格数据对你更有参考价值。

### 电信

辽宁电信到Digital Ocean各机房的测速数据 [20180804] 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率 | 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率  
---|---|---|---|---|---|---|---|---|---  
法兰克福 | 84个 | 0 | 226ms | 0 | 均值 | 980个 | 21个 | 323ms | 8%  
旧金山 | 152个 | 3个 | 340ms | 10% | 纽约 | 240个 | 3个 | 337ms | 9%  
伦敦 | 84个 | 0 | 280ms | 0 | 班加罗尔 | 84个 | 3个 | 380ms | 1%  
多伦多 | 84个 | 0 | 284ms | 1% | 新加坡 | 84个 | 6个 | 436ms | 18%  
阿姆斯特丹 | 168个 | 6个 | 290ms | 17% |  |  |  |  |   
  
简评：如果你是来自辽宁的 **电信** 用户，想运行Digital Ocean的VPS产品，用作科学上网、云存储、远程任务、游戏服务器等用途，建议重点关注部署在 **法兰克福** 的机房；从数据看，你的网络到这里的连通速度、丢包率都比较不错。文章结尾处有本文视角的其他近期测评的链接，建议你多看几期以提升判断的准确度；如果看好了，就果断[去让机器跑起来](https://vps123.top/go/do/_1)吧！

### 联通

辽宁联通到Digital Ocean各机房的测速数据 [20180804] 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率 | 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率  
---|---|---|---|---|---|---|---|---|---  
法兰克福 | 86个 | 3个 | 262ms | 1% | 均值 | 1101个 | 12个 | 293ms | 4%  
旧金山 | 185个 | 6个 | 246ms | 6% | 纽约 | 274个 | 0 | 304ms | 2%  
伦敦 | 90个 | 0 | 277ms | 2% | 班加罗尔 | 90个 | 0 | 316ms | 2%  
多伦多 | 98个 | 0 | 298ms | 4% | 新加坡 | 90个 | 0 | 393ms | 11%  
阿姆斯特丹 | 188个 | 3个 | 282ms | 3% |  |  |  |  |   
  
简评：如果你是来自辽宁的 **联通** 用户，想运行Digital Ocean的VPS产品，用作科学上网、云存储、远程任务、游戏服务器等用途，请慎重选择部署在 **旧金山** 的机房，即使此处离你较近；因为响应均值虽然还不错，但丢包率较高。文章结尾处有本文视角的其他近期测评的链接，建议你多看几期以提升判断的准确度；如果看好了，就果断[去让机器跑起来](https://vps123.top/go/do/_2)吧！

### 移动

辽宁移动到Digital Ocean各机房的测速数据 [20180804] 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率 | 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率  
---|---|---|---|---|---|---|---|---|---  
法兰克福 | 36个 | 0 | 303ms | 6% | 均值 | 428个 | 0 | 280ms | 4%  
旧金山 | 68个 | 0 | 245ms | 5% | 纽约 | 108个 | 0 | 290ms | 4%  
伦敦 | 36个 | 0 | 284ms | 3% | 班加罗尔 | 36个 | 0 | 302ms | 2%  
多伦多 | 36个 | 0 | 280ms | 3% | 新加坡 | 36个 | 0 | 220ms | 6%  
阿姆斯特丹 | 72个 | 0 | 304ms | 6% |  |  |  |  |   
  
简评：如果你是来自辽宁的 **移动** 用户，想运行Digital Ocean的VPS产品，用作科学上网、云存储、远程任务、游戏服务器等用途，建议重点关注部署在 **旧金山** 的机房；从数据看，你的网络到这里的连通速度、丢包率都比较不错。请慎重选择部署在 **新加坡** 的机房，即使此处离你较近；因为响应均值虽然还不错，但丢包率较高。文章结尾处有本文视角的其他近期测评的链接，建议你多看几期以提升判断的准确度；如果看好了，就果断[去让机器跑起来](https://vps123.top/go/do/_3)吧！
...
## 相关测速报告
  ...
  * 辽宁到Digital Ocean各机房的 _其他近期报告_ ： 
    * [20180514](/digitalocean/isp/liaoning/20180514-digitalocean-isp-liaoning.md "辽宁到Digital Ocean各机房的Ping测试 20180514")
  ...
  * 本期报告：辽宁到 _其他VPS提供商_ 各机房的测速报告： 
    * [Vultr](/vultr/isp/liaoning/20180804-vultr-isp-liaoning.md "辽宁到Vultr各机房的Ping测试 20180804")
...
本文最初发表于[辽宁到Digital Ocean各机房的测速数据（20180804）](https://vps123.top/pingtest/20180804-digitalocean-isp-liaoning.html)
