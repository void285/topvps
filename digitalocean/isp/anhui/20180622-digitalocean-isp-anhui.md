#  安徽到Digital Ocean各机房的测速数据（20180622） 

## 导读

本文是[TopVPS主机网络测试报告系列](https://vps123.top/pingtest)的一部分，主要提供PING测试的响应速度、丢包率等数据，关于PING指令以及响应速度、丢包率的指标意义，请看[这里](https://vps123.top/what-is-ping.html)。

![安徽到Digital Ocean各机房的测速数据（20180622）](/images/thumbnails/Anhui_to_digitalocean.png)
...
测速点的运营商线路为电信、联通、移动，有效样本点374个，其中超时点10个。连通概况如下：大陆线路响应均值为270毫秒，最快的三个机房所在地为旧金山、多伦多、新加坡，最慢的三个为阿姆斯特丹、纽约、班加罗尔；旧金山、阿姆斯特丹、纽约、多伦多、法兰克福等7处有响应超时情况；新加坡丢包率较高。

[注册 Digital Ocean](https://vps123.top/go/do/_btn1)

## 安徽图表数据

![大陆省份安徽到VPS提供商Digital Ocean各机房的ping测试数据统计图，包含响应值的柱状图以及丢包率的散点图，数据日期为20180622](/images/pingtests/do_20180622/plot_isp_anhui_do_20180622.png)

### 全部运营商

安徽全部运营商到Digital Ocean各机房的测速数据 [20180622] 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率 | 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率  
---|---|---|---|---|---|---|---|---|---  
旧金山 | 73个 | 2个 | 232ms | 3% | 伦敦 | 36个 | 1个 | 278ms | 0  
多伦多 | 36个 | 1个 | 247ms | 0 | 阿姆斯特丹 | 70个 | 2个 | 279ms | 1%  
新加坡 | 9个 | 0 | 264ms | 20% | 纽约 | 75个 | 2个 | 294ms | 1%  
法兰克福 | 37个 | 1个 | 267ms | 0 | 班加罗尔 | 38个 | 1个 | 322ms | 2%  
均值 | 374个 | 10个 | 270ms | 1% |  |  |  |  |   
  
简评：本表展示了安徽的 **电信、联通、移动** 线路到Digital Ocean各机房的平均测速数据，这些数据绘制在前面柱状图的 **绿色** 柱位上；如果你常驻安徽，可从此表了解你常用的网络到Digital Ocean各个机房的平均响应速度以及丢包率。从表中数据可以看到， **旧金山、多伦多** 的响应速度小于250ms，且丢包率在5%以内，值得关注；如果你多数时间只使用某运营商的网络，后面的分运营商的表格数据对你更有参考价值。

### 电信

安徽电信到Digital Ocean各机房的测速数据 [20180622] 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率 | 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率  
---|---|---|---|---|---|---|---|---|---  
旧金山 | 31个 | 0 | 241ms | 5% | 伦敦 | 17个 | 0 | 225ms | 0  
多伦多 | 17个 | 0 | 230ms | 0 | 阿姆斯特丹 | 31个 | 0 | 251ms | 0  
新加坡 | 5个 | 0 | 301ms | 34% | 纽约 | 35个 | 0 | 298ms | 1%  
法兰克福 | 16个 | 0 | 235ms | 0 | 班加罗尔 | 17个 | 0 | 389ms | 2%  
均值 | 169个 | 0 | 267ms | 2% |  |  |  |  |   
  
简评：如果你是来自安徽的 **电信** 用户，想运行Digital Ocean的VPS产品，用作科学上网、云存储、远程任务、游戏服务器等用途，建议重点关注部署在 **伦敦、多伦多、法兰克福、旧金山** 的机房；从数据看，你的网络到这几处的连通速度、丢包率都比较不错。文章结尾处有本文视角的其他近期测评的链接，建议你多看几期以提升判断的准确度；如果看好了，就果断[去让机器跑起来](https://vps123.top/go/do/_1)吧！

### 联通

安徽联通到Digital Ocean各机房的测速数据 [20180622] 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率 | 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率  
---|---|---|---|---|---|---|---|---|---  
旧金山 | 16个 | 2个 | 232ms | 1% | 伦敦 | 8个 | 1个 | 300ms | 0  
多伦多 | 8个 | 1个 | 256ms | 0 | 阿姆斯特丹 | 15个 | 2个 | 296ms | 1%  
新加坡 | 1个 | 0 | 266ms | - | 纽约 | 15个 | 2个 | 310ms | 0  
法兰克福 | 7个 | 1个 | 299ms | 0 | 班加罗尔 | 8个 | 1个 | 292ms | 2%  
均值 | 78个 | 10个 | 275ms | 1% |  |  |  |  |   
  
简评：如果你是来自安徽的 **联通** 用户，想运行Digital Ocean的VPS产品，用作科学上网、云存储、远程任务、游戏服务器等用途，建议重点关注部署在 **旧金山** 的机房；从数据看，你的网络到这里的连通速度、丢包率都比较不错。文章结尾处有本文视角的其他近期测评的链接，建议你多看几期以提升判断的准确度；如果看好了，就果断[去让机器跑起来](https://vps123.top/go/do/_2)吧！

### 移动

安徽移动到Digital Ocean各机房的测速数据 [20180622] 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率 | 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率  
---|---|---|---|---|---|---|---|---|---  
旧金山 | 26个 | 0 | 223ms | 2% | 伦敦 | 11个 | 0 | 307ms | 0  
多伦多 | 11个 | 0 | 256ms | 0 | 阿姆斯特丹 | 24个 | 0 | 290ms | 3%  
新加坡 | 3个 | 0 | 226ms | 7% | 纽约 | 25个 | 0 | 275ms | 1%  
法兰克福 | 14个 | 0 | 266ms | 0 | 班加罗尔 | 13个 | 0 | 284ms | 3%  
均值 | 127个 | 0 | 267ms | 2% |  |  |  |  |   
...
本文最初发表于[安徽到Digital Ocean各机房的测速数据（20180622）](https://vps123.top/pingtest/20180622-digitalocean-isp-anhui.html)
