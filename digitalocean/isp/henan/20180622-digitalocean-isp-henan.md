#  河南到Digital Ocean各机房的测速数据（20180622） 
...
河南全部运营商到Digital Ocean各机房的测速数据 [20180622] 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率 | 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率  
---|---|---|---|---|---|---|---|---|---  
新加坡 | 11个 | 2个 | 209ms | 4% | 伦敦 | 43个 | 0 | 273ms | 0  
旧金山 | 98个 | 3个 | 209ms | 3% | 法兰克福 | 50个 | 1个 | 289ms | 4%  
多伦多 | 45个 | 1个 | 246ms | 2% | 阿姆斯特丹 | 97个 | 3个 | 304ms | 5%  
纽约 | 99个 | 2个 | 264ms | 6% | 班加罗尔 | 51个 | 1个 | 330ms | 3%  
均值 | 494个 | 13个 | 268ms | 4% |  |  |  |  |   
...
河南电信到Digital Ocean各机房的测速数据 [20180622] 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率 | 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率  
---|---|---|---|---|---|---|---|---|---  
新加坡 | 7个 | 1个 | 288ms | 12% | 伦敦 | 14个 | 0 | 263ms | 0  
旧金山 | 32个 | 1个 | 223ms | 0 | 法兰克福 | 16个 | 0 | 284ms | 0  
多伦多 | 14个 | 0 | 249ms | 0 | 阿姆斯特丹 | 32个 | 1个 | 318ms | 5%  
纽约 | 32个 | 1个 | 283ms | 0 | 班加罗尔 | 16个 | 0 | 377ms | 0  
均值 | 163个 | 4个 | 282ms | 1% |  |  |  |  |   
...
河南联通到Digital Ocean各机房的测速数据 [20180622] 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率 | 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率  
---|---|---|---|---|---|---|---|---|---  
新加坡 | 3个 | 1个 | 244ms | 0 | 伦敦 | 15个 | 0 | 298ms | 0  
旧金山 | 37个 | 0 | 193ms | 9% | 法兰克福 | 18个 | 0 | 301ms | 8%  
多伦多 | 17个 | 1个 | 233ms | 7% | 阿姆斯特丹 | 33个 | 0 | 311ms | 5%  
纽约 | 37个 | 0 | 233ms | 14% | 班加罗尔 | 20个 | 0 | 325ms | 10%  
均值 | 180个 | 2个 | 260ms | 8% |  |  |  |  |   
  
简评：如果你是来自河南的 **联通** 用户，想运行Digital Ocean的VPS产品，用作科学上网、云存储、远程任务、游戏服务器等用途，建议重点关注部署在 **新加坡** 的机房；从数据看，你的网络到这里的连通速度、丢包率都比较不错。请慎重选择部署在 **旧金山、多伦多、纽约** 的机房，即使这几个城市离你较近；因为响应均值虽然还不错，但丢包率较高。文章结尾处有本文视角的其他近期测评的链接，建议你多看几期以提升判断的准确度；如果看好了，就果断[去让机器跑起来](https://vps123.top/go/do/_2)吧！

### 移动

河南移动到Digital Ocean各机房的测速数据 [20180622] 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率 | 机房所在地 | 测速点 | 超时点 | 响应均值 | 丢包率  
---|---|---|---|---|---|---|---|---|---  
新加坡 | 1个 | 0 | 96ms | 0 | 伦敦 | 14个 | 0 | 258ms | 1%  
旧金山 | 29个 | 2个 | 212ms | 0 | 法兰克福 | 16个 | 1个 | 280ms | 3%  
多伦多 | 14个 | 0 | 257ms | 0 | 阿姆斯特丹 | 32个 | 2个 | 284ms | 5%  
纽约 | 30个 | 1个 | 277ms | 4% | 班加罗尔 | 15个 | 1个 | 288ms | 0  
均值 | 151个 | 7个 | 262ms | 2% |  |  |  |  |   
